...
object ProtocPlugin extends AutoPlugin {
  ...
  private[this] def compile(
    ...
    val srcManagedMainDir = "/src_managed/main"
    val srcManagedTestDir = "/src_managed/test"

    def isSrcManagedSolely(f: File): Boolean = {
      val path = f.getCanonicalPath.replace('\\', '/')

      path.endsWith(srcManagedMainDir) || path.endsWith(srcManagedTestDir)
    }

    if (deleteTargetDirectory) {
      targetPaths.foreach {
        case targetPath if isSrcManagedSolely(targetPath) =>
          def fileList               = (targetPath ** "*.scala").get().toSet[File]
          val toDel: File => Boolean = f =>
            IO.read(f)
              .dropWhile(_.isWhitespace)
              .startsWith("// Generated by the Scala Plugin for the Protocol Buffer Compiler.")

          fileList.foreach(f => if (toDel(f)) IO.delete(f))
